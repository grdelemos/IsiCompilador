...
semantiC :: Tree -> [Var] -> (String, String)

semantiC (NoProg declara blocos) symTab = 
   let 
    ...
    case t of 
        (NoDeclaraNum x) ->
            let (num, symTab') = declararNumero x symTab
              ...
              ("float " ++ num ++ prox, symTab'') 
        (NoDeclaraTxt x) ->
            let (txt, symTab') = declararTexto x symTab 
                (prox', symTab'') = declarar ts symTab' in
              ("char " ++ txt ++ prox', symTab'')
        (NoDeclara var) ->
            declarar var symTab
...
declararTexto (t : ts) symTab =
    case t of NoId a ->
                        ...
                        if foiDeclarada a symTab' then error $ "Variável " ++ show a ++ " declarada mais de uma vez" else
                        if null prox then (a ++ "[100];\n", declarada) else (a ++ "[100], " ++ prox, declarada)
    ...
    case expr of
        ...
        NoPow exp exp' ->
            let (expressao, symTab') = avalExpress exp symTab
                (expressao', symTab'') = avalExpress exp' symTab' in
                    ("pow(" ++ expressao ++ ", " ++ expressao' ++ ")", symTab'')
        NoPowParen exp exp' ->
            let (expressao, symTab') = avalExpress exp symTab
                (expressao', symTab'') = avalExpress exp' symTab' in
                    ("(" ++ "pow(" ++ expressao ++ ", " ++ expressao' ++ ")" ++ ")", symTab'')
